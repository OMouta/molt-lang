...
func TestParsePrimaryFormsAndSequences(t *testing.T) {
	...
	if importExpr.Kind != ast.ImportModule {
		t.Fatalf("import kind = %v, want ImportModule", importExpr.Kind)
	}
	if importExpr.Name == nil || importExpr.Name.Name != "lib" {
		name := "<nil>"
		if importExpr.Name != nil {
			name = importExpr.Name.Name
		}
		t.Fatalf("import alias = %q, want %q", name, "lib")
	}
...
func TestParseRejectsMalformedPrograms(t *testing.T) {
	tests := []struct {
		...
		{name: "import named missing from", input: "import x", message: "expected 'from' after import names"},
		{name: "import named missing path", input: "import x from", message: "expected string literal after 'from'"},
		{name: "import module missing alias", input: "import \"./lib.molt\" as", message: "expected identifier after 'as'"},
